...
clearvars 
% Working Directory
cd 'C:\Users\33678\Desktop\Deeqa_EEE\Empirical industrial organization 1\Part 1 Isis\Homework\bin_2020'
% Import the data
Data_Initial = csvread("base_project_deeqa_notext.csv");
...
X0 = Data_Initial(:,5:8); % ,Cylinder, Weight, Horsepower
...
for i = 1:nb_grid
    obj_value(i) = gmm_blp(grid(i), sj, X0, Endo, Endo, Z, Inverse_ZTZ, nu, delta_init, yearStarts, yearEnds);
end
...
sigma_hat_Nfe = fminunc(@(sigma)gmm_blp(sigma, sj, X0, Endo, Endo, Z, Inverse_ZTZ, nu, delta_init, ...
...
for tt = 1: T %for each time 2003-2008
    Jt = yearStarts(tt):yearEnds(tt);  
    [delta_nofe(Jt), pb_cv(tt)] = blp_contraction(sigma_hat_Nfe, sj(Jt), delta_init(Jt), nu,Endo(Jt,:));   
end
...
X0_fe = [X0 brand_fe year_fe];
Z_fe = [X0_fe Data_Initial(:,11:22)];
...
for i = 1:nb_grid_fe
    obj_value_fe(i) = gmm_blp(grid_fe(i), sj, X0_fe, Endo, Endo, Z_fe, Inverse_ZTZ_fe, nu, delta_init, yearStarts, yearEnds);
end
...
sigma_hat_fe = fminunc(@(sigma)gmm_blp(sigma, sj, X0_fe, Endo, Endo, Z_fe, Inverse_ZTZ_fe, nu, delta_init, ...
...
for tt = 1: T %for each time 2003-2008
    Jt = yearStarts(tt):yearEnds(tt);  
    [delta_fe(Jt), pb_cv_fe(tt)] = blp_contraction(sigma_hat_fe, sj(Jt), delta_init(Jt), nu,Endo(Jt));   
end
...
numer = exp(delta_fe_2008  + sigma_hat_fe*price_2008*nu);
...
Temp_omega1 = diag(sum((beta_hat_fe(end)+sigma_hat_fe*nu).*s_hat_ij,2));
Temp_omega2 = (beta_hat_fe(end)+sigma_hat_fe*nu).*s_hat_ij*s_hat_ij';
...
Sum_cs = consumer(delta_fe_2008,alpha_hat_fe, price_2008, sigma_hat_fe,nu,ns)*10000;
