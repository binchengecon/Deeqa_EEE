...
clearvars 
% Working Directory
cd 'C:\Users\33678\Desktop\Deeqa_EEE\Empirical industrial organization 1\Part 1 Isis\Homework\Solution'
% Import the data
...
delta_Nfe = zeros(kt,1);
for tt = 1: T %for each time 2003-2008
    Jt = yearStarts(tt):yearEnds(tt);  
    [delta_Nfe(Jt), pb_cv(tt)] = blp_contraction(sigma_hat_Nfe, sj(Jt), delta_init(Jt), nu,fuel_cost(Jt,:));   
end


% IV regression without FE
[beta_hat_Nfe] = ivregression(delta_Nfe, X0, Endo, Z);

% fuel cost is the second last varibale 

alpha_hat_Nfe = beta_hat_Nfe(5);
...
ReResultVarNames = {'No FE','With Brand FE','With Brand and Year FE'};
% Table_CFResult = renamevars(Table_CFResult,allvars,CFResultNames);
ReResultRowNames = {'Intercept','Cylinder','Weight','Horsepower','Fuel Cost','RC Fuel Cost','Price'};
...
Avg_cs = consumer(delta_fe_2008,alpha_hat_fe, fuel_cost_2008, sigma_hat_fe,nu,ns)*10000;
...
numer = exp(delta_fe_2008_np  +beta_hat_fe(end)*(price_2008_CF+10*co2_2008/10000) + sigma_hat_fe*fuel_cost_2008*nu);
...
profit_temp_CF =  (price_2008_CF-mc_2008).*s_hat_j.*nb_households(Jt_2008)*10000/1000000; 

Sum_profit_CF = sum(profit_temp_CF);

Sum_cs_CF = consumer2(delta_fe_2008_np, price_2008_CF , beta_hat_fe(end) , tax_2008, alpha_hat_fe,fuel_cost_2008,sigma_hat_fe,nu,ns)*max(nb_households(Jt_2008))*10000/1000000;
...
Table_CFResult(2,1) = mean(price_2008);
Table_CFResult(3,1) = mean(1-mc_2008./price_2008);
Table_CFResult(4,1) = mean(price_2008./mc_2008-1);
Table_CFResult(5,1) = Sum_profit;
Table_CFResult(6,1) = Sum_cs;
% Table_CFResult(6,1) = Array_Sum_cs_CF(1);
Table_CFResult(7,1) = 0;
...
Table_CFResult(6,2) = Array_Sum_cs_CF(11)*100;
...
Table_CFResult(6,3) = Array_Sum_cs_CF(Index_Max_TaxR)*100;
...
Table_CFResult(6,4) = Array_Sum_cs_CF(100)*100;
